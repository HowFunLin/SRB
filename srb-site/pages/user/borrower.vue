...
export default {
  data() {
    ...
    return {
      // 状态指示
      active: null, //步骤
      borrowerStatus: null, // 借款人状态（2：审核通过；-1：审核不通过）
      submitBtnDisabled: false, // 是否展示提交按钮
  ...
  created() {
    // this.initSelected(); // 移至未认证时再初始化数据
    this.getUserInfo();
  },
  ...
  methods: {
    ...
    // 下列方法逻辑相似可复用，回调参数由Element-UI默认定义
      ...
      this.$axios
        ...
        .then((response) => {
          this.borrower.borrowerAttachList = this.borrower.borrowerAttachList.filter(
            (item) => {
              // 返回 true 时过滤保留，从 List 中删除 URL 符合条件的 BorrowAttach
