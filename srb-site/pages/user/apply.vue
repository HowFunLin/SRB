...
export default {
  ...
  // 渲染完成之后再获取后端不会报错，而created会报错（created 渲染完成后会重试，不影响最终结果）
  mounted() {
    // 当且仅当表单展示时才获取，方法调用移入 getBorrowInfoStatus() 方法中
  ...
  watch: {
    ...
    "borrowInfo.amount"(value) {
      if (value > this.borrowAmount) {
        this.$alert("您的借款额度不足", {
          type: "error",
          // 点击提示框按钮之后的回调函数，使用箭头函数解决 this 作用域问题
          callback: () => {
